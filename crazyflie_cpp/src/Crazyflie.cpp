    ...
    , m_linkQualityCallback     (nullptr)
{
    ...
    else throw std::runtime_error("Uri is not valid!");
}
...
void Crazyflie::handleAck(const Crazyradio::Ack &result) {
    if (crtpConsoleResponse::match(result)) {
        ...
        if (r->command == 0 && (r->result == crtpLogControlResultOk || r->result == crtpLogControlResultBlockExists))
