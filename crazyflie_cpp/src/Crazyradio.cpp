...
Crazyradio::~Crazyradio () {
    int result = libusb_release_interface (m_handle, 0);
    if (result != LIBUSB_SUCCESS)
...
bool Crazyradio::open (uint32_t devid) {
    ...
    if (found) {
        int err = libusb_open (found, &m_handle);
        if (err != LIBUSB_SUCCESS) {
            std::cerr << libusb_error_name(err) << std::endl;
            libusb_free_device_list (list, 1);
            return false;
    ...
    Ack             &result)
{
    ...
    if (length != transferred)
        std::cerr << "Did transfer " << transferred <<
                     " but " << length << " was requested!" <<
                     std::endl;
    ...
    uint16_t length)
{
    ...
    int status = libusb_control_transfer (
        ...
        /*timeout*/ 1000);
    if (status != LIBUSB_SUCCESS)
        std::cerr << "sendVendorSetup: " << libusb_error_name (status)
                  << std::endl;
}
